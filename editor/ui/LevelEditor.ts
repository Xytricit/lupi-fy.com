...
import { SceneManager } from '../../runtime/core/SceneManager';
import { Entity } from '../../runtime/core/Entity';
import { LevelData, SpawnPoint, TileLayer, TileDefinition } from '../../types/level';
...
class LevelEditor {
  ...
  private selectedTool: 'tile' | 'entity' | 'spawn' | 'eraser' = 'tile';
  ...
  private entityTemplates: Record<string, Entity> = {};
  private tilePalettes: Record<string, TileDefinition[]> = {};
  private undoStack: LevelData[] = [];
  private redoStack: LevelData[] = [];

  constructor(projectData: ProjectData, canvas: HTMLCanvasElement) {
  ...
  private initializeLevel(): LevelData {
    return {
      id: `level_${Date.now()}`,
      name: 'New Level',
      width: 100,
  ...
  private loadAssets() {
    ...
    entityAssets.forEach(asset => {
      if (asset.tags.includes('entity')) {
        this.entityTemplates[asset.id] = new Entity({
          id: asset.id,
          name: asset.name,
          sprite: asset.data,
          width: 32,
  ...
  private initUI() {
    ...
    toolbar.querySelectorAll('.tool-selector button').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (tool) this.selectTool(tool as any);
      });
    });
    ...
    paletteContainer.style.background = '#2c3e50';
    paletteContainer.style.borderRadius = '5px';
    paletteContainer.style.padding = '10px';
    paletteContainer.style.boxShadow = '0 2px 10px rgba(0,0,0,0.3)';
  ...
  private selectTool(tool: 'tile' | 'entity' | 'spawn' | 'eraser') {
  ...
  private renderTilePalette(paletteName: string) {
    const paletteContainer = document.querySelector('.tile-palette') as HTMLElement;
  ...
  private placeSpawnPoint(x: number, y: number) {
  ...
  private drawSelectionGrid() {
    if (this.selectedTool === 'tile' && this.selectedTile) {
      this.ctx.fillStyle = 'rgba(52, 152, 219, 0.3)';
      this.ctx.fillRect(
        Math.floor(this.dragStart.x) * this.gridSize,
        Math.floor(this.dragStart.y) * this.gridSize,
        this.gridSize,
        this.gridSize
      );
    }
  ...
  private drawUIOverlay() {
    ...
    this.ctx.fillText(
      `Level: ${this.currentLevel.name} | Grid: ${this.currentLevel.width}x${this.currentLevel.height} | FPS: ${this.sceneManager.getFPS()}`,
      10,
      25
    );
    
    this.ctx.fillText(
      `Tool: ${this.selectedTool.toUpperCase()}${this.selectedTile ? ` - ${this.selectedTile.name}` : ''}`,
      10,
      50
    );
  ...
  private loadLevelById(levelId: string) {
  ...
  public importLevel(levelData: LevelData) {
    this.undoStack.push(JSON.parse(JSON.stringify(this.currentLevel)));
...
registerBlock({
  ...
  inputs: {
    TYPE: { 
      type: "dropdown", 
      options: ["player", "enemy", "item", "npc", "checkpoint"],
      default: "player"
    },
...
registerBlock({
  ...
  inputs: {
    TYPE: { 
      type: "dropdown", 
      options: ["player", "enemy", "item", "npc", "checkpoint", "random"],
      default: "player"
    }
  },
  output: "vector",
